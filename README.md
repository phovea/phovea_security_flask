...
**NOTE:** If there are any credentials defined via environment variables, those from the config.json will not be used. This behaviour can be used to have development users inside the config.json, and provide the users for production via environment variables when deployed.
